 ...
 */
export async function scrapeDuckDuckGoShopping(
  searchQuery: string,
  ...
  try {
    ...
    const products = await page.evaluate((maxResults: number) => {
      ...
      for (let i = 0; i < Math.min(productElements.length, maxResults); i++) {
        ...
        const result: Partial<ProductResult> = {};
        ...
        if (result.title) {
          results.push(result as ProductResult);
        }
      }
    ...
    if (results.length === 0) {
      ...
      const fallbackResults = await page.evaluate((maxResults: number) => {
        const results: ProductResult[] = [];
        if (typeof document === 'undefined') {
          return results;
        }
